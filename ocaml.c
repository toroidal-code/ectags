...
/*//////////////////////////////////////////////////////////////////////
//// Lexing					*/
static boolean isNum(char c) { return c >= '0' && c <= '9'; }
static boolean isLowerAlpha(char c) { return c >= 'a' && c <= 'z'; }

static boolean isUpperAlpha(char c) { return c >= 'A' && c <= 'Z'; }

static boolean isAlpha(char c) { return isLowerAlpha(c) || isUpperAlpha(c); }
...
static void jumpToMatchContext(void) {
  ...
  for (i = stackIndex; i >= 0; --i) {
    if (stack [i].type == ContextMatch) {
      stackIndex = i + 1;      
      toDoNext = stack[i].callback; // this should always be matchPattern
...
/* used to prepare an OCaml tag , just in case their is a need to
 * add additional information to the tag. */
static void prepareTag(tagEntryInfo *tag, vString const *name, ocamlKind kind) {
    int parentIndex;

    /* Ripped out of read.h initTagEntry, because of line number shenanigans.
...
static void mayRedeclare(vString *const ident, ocaToken what,
    ...
    case OcaKEYWORD_object:
        ...
        pushContext(ContextStrong, ContextClass, &localScope,
                    NULL /*voidName */);
        needStrongPoping = FALSE;
        toDoNext = &globalScope;
        break;
...
static void globalScope(vString *const UNUSED(ident), ocaToken what,
    ...
    case OcaKEYWORD_end:;
        ...
        /** so here, end can legally be followed by = or and in the situation of
          * module M : sig ... end = struct ... end  and
          * module M struct ... end and N = struct ... end
          * and we need to make sure we know we're still inside of a struct */
...
static void localScope(vString *const ident, ocaToken what, ocaToken whatNext) {
    ...
    case OcaKEYWORD_and:
        if (toDoNext == &mayRedeclare) {
            ...
            /* a local 'and' keyword jumps up a context to the last named. For ex
             * in `with let IDENT ... and IDENT2 ...` ident and ident2 are on 
             * same level, the same as `let IDENT ... in let IDENT2 ...`
             * a 'let' is the only 'and'-chainable construct allowed locally
             * (thus we had to be one to get here), so we either go to
...
/* clang's -licm (Loop Invariant Code Motion) optimization pass actually
 * alters the main while loop. This has the (inadvertant and incorrect)
 * side-effect of moving the line/filePos update statements _outside_ the loop,
 * because it never sees them get updated inside of it ('File' is in another
 * object file). On GCC and versions of clang bundled with LLVM 3.5 and up,
 * the 'optnone' attribute is respected, and everything works predictably.
 * ... I'm still going to email the clang dev team about this. */
__attribute__((optnone)) static void findOcamlTags(void) {
